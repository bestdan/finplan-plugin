...
| `spouse_json`        | dict   | Spouse profile (same fields as a person). Omitting `id` keeps the existing spouse's id; pass an explicit `id` to swap in a different person. The primary person must be married. Required for: set_spouse.                            |
