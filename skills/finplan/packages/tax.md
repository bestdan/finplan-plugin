...
Analyze converting a traditional IRA/401(k) amount to Roth this year: the marginal federal tax cost of the conversion slice (two full-liability calls diffed, so bracket-stacking, NIIT, and AMT are captured), the resulting reduction in projected RMDs, and a convert-now vs leave-in-traditional break-even under an assumed future marginal rate. The conversion amount is given either explicitly (`conversion_amount_cents`) or by filling a target ordinary bracket (`fill_to_marginal_rate`) — provide exactly one. The break-even assumes the conversion tax is paid from the converted (IRA) funds. Out of scope for v1: IRMAA tiers, ACA subsidy cliffs, state/local tax interaction, multi-year conversion ladders.
...
Returns: `assumptions` (echo of inputs, resolved `conversion_amount`, `conversion_mode`, and `notes`), `incremental_tax_cost_cents`/`_dollars`, `marginal_cost_rate`, `baseline_marginal_ordinary_rate`, `post_conversion_marginal_ordinary_rate`, `rmd_reduction` (per-year `full_rmd`/`reduced_rmd`/`reduction`), `total_rmd_reduction_cents`/`_dollars`, `break_even` (`convert_now_terminal`, `leave_in_traditional_terminal`, `advantage`, `break_even_future_rate`, `recommendation`), `summary`.
