...
Analyze converting a traditional IRA/401(k) amount to Roth this year: the marginal federal tax cost of the conversion slice (two full-liability calls diffed, so bracket-stacking, NIIT, and AMT are captured), the resulting reduction in projected RMDs, and a convert-now vs leave-in-traditional break-even under an assumed future marginal rate. The conversion amount is given either explicitly (`conversion_amount_cents`) or by filling a target ordinary bracket (`fill_to_marginal_rate`) — provide exactly one. The break-even assumes the conversion tax is paid from the converted (IRA) funds. Provide `state_code` (with the `state_*` income components) to fold resident state+local income tax on the conversion into the cost and break-even; unsupported states return `success: false`. Out of scope: IRMAA tiers, ACA subsidy cliffs, multi-year conversion ladders.
