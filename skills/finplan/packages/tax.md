...
Returns: `assumptions` (echo of inputs, resolved `conversion_amount`, `conversion_mode`, `notes`, and — when `state_code` is set — a `state` block with the resident state/local assumptions and resolved future rates), `incremental_tax_cost_cents`/`_dollars` (federal), `marginal_cost_rate`, `baseline_marginal_ordinary_rate`, `post_conversion_marginal_ordinary_rate`, `rmd_reduction` (per-year `full_rmd`/`reduced_rmd`/`reduction`), `total_rmd_reduction_cents`/`_dollars`, `break_even` (`convert_now_terminal`, `leave_in_traditional_terminal`, `advantage`, `break_even_future_rate`, `recommendation`), `summary`. When `state_code` is set, also `state_incremental_tax_cost_cents`/`_dollars`, `state_marginal_cost_rate`, `combined_incremental_tax_cost_cents`/`_dollars`, and `combined_marginal_cost_rate` (federal+state); the `break_even` then reflects the combined cost.
