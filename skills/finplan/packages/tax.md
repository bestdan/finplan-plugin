...
| `foreign_earned_income_exclusion_cents`    | int    | IRC §911 foreign-earned-income exclusion in cents (default `0`). Added back to reach MAGI for the NIIT and the SALT phase-down; must be excluded from `wages_cents` |
...
Analyze converting a traditional IRA/401(k) amount to Roth this year: the marginal federal tax cost of the conversion slice (two full-liability calls diffed, so bracket-stacking, NIIT, and AMT are captured), the resulting reduction in projected RMDs, and a convert-now vs leave-in-traditional break-even under an assumed future marginal rate. The conversion amount is given either explicitly (`conversion_amount_cents`), by filling a target ordinary bracket (`fill_to_marginal_rate`), or optimized (`recommend`, "how much should I convert?") — provide exactly one. Recommend mode returns the largest amount whose next-dollar marginal cost rate stays at or below `future_marginal_rate` (0 when converting nothing is best); it is federal-only and cliff-unaware (does not stop short of an IRMAA/ACA surcharge edge). The break-even assumes the conversion tax is paid from the converted (IRA) funds. Provide `state_code` (with the `state_*` income components) to fold resident state+local income tax on the conversion into the cost and break-even; unsupported states return `success: false`. Out of scope: IRMAA tiers, ACA subsidy cliffs, multi-year conversion ladders.
...
| `conversion_amount_cents`                    | int    | Explicit conversion amount in cents. Provide exactly one of `conversion_amount_cents` / `fill_to_marginal_rate` / `recommend` (optional)                                                                                                                                                                |
| `fill_to_marginal_rate`                      | float  | Fill-bracket mode: convert enough to raise ordinary taxable income to the top of the bracket with this marginal rate, e.g. `0.24`. Provide exactly one of `conversion_amount_cents` / `fill_to_marginal_rate` / `recommend` (optional)                                                                  |
| `recommend`                                  | bool   | Recommend mode: return the largest conversion whose next-dollar marginal cost rate stays at or below `future_marginal_rate` (0 = convert nothing). Federal-only, cliff-unaware. Provide exactly one of `conversion_amount_cents` / `fill_to_marginal_rate` / `recommend` (default `false`)              |
...
| `state_wages_cents`                          | int    | Wages, salary, and other compensation in cents, after any state deduction the caller applies. Part of the state income base. Every `state_*` component defaults to 0; `state_code` with all of them left at 0 is accepted as a (degenerate) all-zero state base (optional)                              |
| `state_social_security_cents`                | int    | Social Security benefits in cents. Most states exempt these (optional)                                                                                                                                                                                                                                  |
| `state_pension_cents`                        | int    | Pension and annuity income in cents, before the conversion. On a joint return this is the taxpayer's; put the spouse's in `state_spouse_pension_cents` (optional)                                                                                                                                       |
| `state_retirement_distribution_cents`        | int    | IRA / 401(k) / 403(b) distributions already received this year, before the conversion, in cents. The conversion amount itself is added automatically as an additional taxpayer retirement distribution — it is not entered here (optional)                                                              |
| `state_interest_dividends_cents`             | int    | Taxable interest and dividend income in cents (optional)                                                                                                                                                                                                                                                |
| `state_capital_gains_cents`                  | int    | Net capital gains in cents (optional)                                                                                                                                                                                                                                                                   |
| `state_business_income_cents`                | int    | Sole-proprietorship, partnership, and S-corp income in cents (optional)                                                                                                                                                                                                                                 |
| `state_other_income_cents`                   | int    | State ordinary income not covered by the other `state_*` fields, in cents. No state exclusion attaches to it (optional)                                                                                                                                                                                 |
| `state_age`                                  | float  | Taxpayer age at the end of the tax year, for age-gated state retirement exclusions (e.g. NY's pension exclusion requires 59.5). This also gates the exclusion for the converted amount itself, since it is classified as a retirement distribution. Leaving this at 0 forfeits the exclusion (optional) |
| `state_spouse_age`                           | float  | Spouse's age at the end of the tax year, on a joint return. Ignored unless `filing_status` is `married_joint` (optional)                                                                                                                                                                                |
| `state_spouse_pension_cents`                 | int    | Spouse's pension and annuity income in cents, on a joint return (optional)                                                                                                                                                                                                                              |
| `state_spouse_retirement_distribution_cents` | int    | Spouse's IRA / 401(k) / 403(b) distributions in cents, on a joint return, before the conversion. The conversion is attributed to the taxpayer, not the spouse; see `state_retirement_distribution_cents` (optional)                                                                                     |
| `state_federal_agi_cents`                    | int    | Federal AGI in cents, for state exclusions with an income cutoff. 0 means not supplied (optional)                                                                                                                                                                                                       |
| `state_federal_taxable_income_cents`         | int    | Federal taxable income in cents, for states that start from it rather than from AGI. 0 means not supplied (optional)                                                                                                                                                                                    |
| `state_federal_tax_paid_cents`               | int    | Federal income tax liability in cents, for the states that subtract it (OR, AL, MO). 0 means not supplied (optional)                                                                                                                                                                                    |
