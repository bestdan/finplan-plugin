...
2. Reconstruct the most recent **complete** state from the current conversation context:
   - If the most recent `manage_state` result is a **full document** (an `action="create"` result, or any `manage_state` call made with `return_full_state=True`), use it directly.
